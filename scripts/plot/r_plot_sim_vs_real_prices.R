...
a<-ggplot(data=toplot)+theme_bw()+coord_cartesian(xlim=c(1993,2013),ylim=c(1,2.5))+labs(title="Weighted world price",subtitle="\\hskip4mm $\\spadesuit$ simulation \\hskip4mm \\color{green!55!black} \\huge \\raisebox{-0.8mm}{$\\bullet$} \\normalsize \\color{black} real",y="normalized annual price")+theme(panel.border = element_rect(color="black",size=1),panel.grid.major=element_line(colour="black",linetype="dotted"),plot.title = element_text(hjust = 0.5))
a<-a+geom_line(aes(x=time,y=wpw),col="green4")

a<-a+geom_line(data=sim.prices.yearly_1,aes(x=time,y=price),col="black")
...
fileNameNoExt<-"fig_sim_vs_real_prices_annual"
fileNameExt<-".tex"
